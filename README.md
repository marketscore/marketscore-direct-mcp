...
- ИИ-выводы: прогноз расхода, аудит аккаунта со score 0–100, диагностика просадок, готовый еженедельный бриф
- Для агентств — портфельный обзор по всем аккаунтам

**Надёжность и безопасность:**
- Полное покрытие Direct API v5 — частые объекты как удобные инструменты
- Действия на запись помечены отдельно — видно, что именно меняется перед правкой
- Подключение по OAuth: не нужно вручную генерировать и хранить токен Директа
- Корректная обработка ошибок Direct API (в том числе «тихих» — когда API отвечает 200, но правка не применилась)

**Чем отличается от обычных обёрток над Direct API:** кроме доступа к API — слой ИИ-выводов (автопилот, аудит, диагностика, прогноз, бриф), веб-дашборд, Telegram-бот для владельца бизнеса и портфельный обзор по нескольким аккаунтам.
